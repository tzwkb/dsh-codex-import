...
import { runImport, listConversations, formatLocalMinute, DEFAULT_MAX_TEXT_CHARS } from './convert.js'
import { auditSessionsRoot, CONTEXT_ADVISORY_TOKENS as AUDIT_ADVISORY_TOKENS } from './session-audit.js'
...
/**
 * Token count above which an imported history is reported as risky.
 *
 * The harness advertises a 1M-token window for the DeepSeek models and
 * compacts at 20% of it, so a history anywhere near the window can be
 * summarised but leaves little room to work. `dsh-compaction-basic` has to
 * replay a span to summarise it, which means a history at or beyond the window
 * is the one shape that cannot be repaired after the fact — warning here is the
 * only place the problem can still be avoided cheaply. The audit owns the
 * number so an import warning and an audit finding can never disagree.
 */
const CONTEXT_ADVISORY_TOKENS = AUDIT_ADVISORY_TOKENS

/** Render a token count compactly for a terminal report. */
...
const USAGE = [
  ...
  '  --max-text-chars N   keep at most N chars of any single text, message or',
  `                       reasoning block (default ${DEFAULT_MAX_TEXT_CHARS}; 0 disables)`,
  '  --full-history       replay every Codex turn instead of the current window;',
  '                       only for conversations small enough to fit one context',
  '  --audit              measure installed sessions and flag histories too large',
  '                       to compact, instead of importing anything',
  ...
  '',
  'By default a conversation is imported as Codex itself last held it: the latest',
  'compaction window plus everything after it. A rollout replays every turn ever',
  'taken, which for a long conversation is many times larger than the context the',
  'model can still accept — and a history that large can no longer be compacted,',
  'because compaction has to replay the very span that does not fit. Use',
  '--full-history when you want the entire replay anyway.',
  '',
...
function parseInput(rawInput) {
  const opts = {
    ...
    maxTextChars: DEFAULT_MAX_TEXT_CHARS, fullHistory: false, audit: false,
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    } else if (a === '--max-text-chars') {
      opts.maxTextChars = Number(required(i++, a)); opts.explicitAction = true
    } else if (a === '--full-history') {
      opts.fullHistory = true; opts.explicitAction = true
    } else if (a === '--audit') {
      opts.audit = true; opts.explicitAction = true
    } else if (a === '--dry-run') {
  ...
  if (!Number.isSafeInteger(opts.maxTextChars) || opts.maxTextChars < 0) throw new Error('--max-text-chars must be zero or a positive integer')
...
/**
 * Render the `--audit` report for the live sessions root.
 *
 * A session that is well-formed can still be unusable: the harness cannot send
 * a history larger than the model window, and it cannot compact one either,
 * because condensing a span means replaying that span to the summarizer. Those
 * sessions look healthy in the resume list and then refuse every new turn, so
 * naming them is the only way a user can find them.
 */
function auditReport() {
  const root = sessionsRoot()
  const report = auditSessionsRoot(root)
  const lines = [
    `Sessions root: ${root}`,
    `${report.sessions.length} session(s), ${formatTokens(report.totalTokens)} tokens of model context in total.`,
  ]
  if (report.oversized.length === 0) {
    lines.push('', `No session exceeds the ~${formatTokens(CONTEXT_ADVISORY_TOKENS)} token advisory.`)
    return lines.join('\n')
  }
  lines.push('', `Over the ~${formatTokens(CONTEXT_ADVISORY_TOKENS)} token advisory — these cannot be compacted:`)
  for (const session of report.oversized.slice(0, 20)) {
    lines.push(`  ~${formatTokens(session.estimatedTokens).padStart(6)}  ${session.id}  ${session.cwd ?? '-'}`)
  }
  if (report.oversized.length > 20) lines.push(`  …and ${report.oversized.length - 20} more.`)
  lines.push(
    '',
    'Compaction condenses a span by sending that span to the summarizer, so a history at or',
    'beyond the model window can neither be sent nor condensed. Rebuild one from its Codex',
    'conversation — the import now reads Codex\'s current window instead of the whole replay:',
    `  /import-codex --session ${report.oversized[0].id.replace(/^session-/, '')} --force`,
    '--force is required because the installed log is no longer exactly what the importer wrote.',
  )
  return lines.join('\n')
}
...
async function runCommand(opts, ctx, signal) {
  if (opts.audit === true) return auditReport()
  ...
  try {
    ...
    const totals = results.reduce((a, r) => ({
      ...
      truncated: 0, clamped: 0, windowed: 0, imagesImported: 0, imagesSkipped: 0,
    })

    const heaviest = results.reduce((max, r) => Math.max(max, r.estimatedTokens ?? 0), 0)
    const overBudget = results.filter((r) => (r.estimatedTokens ?? 0) > CONTEXT_ADVISORY_TOKENS)
    const lines = [
      ...
      `  Largest imported history: ~${formatTokens(heaviest)} tokens of model context`,
    ]
    if (totals.windowed > 0) {
      lines.push(
        `  ${totals.windowed} conversation(s) imported as Codex's current compaction window; `
        + 'earlier turns stay behind in the rollout and are summarised by Codex\'s own checkpoint',
      )
    }
    if (totals.clamped > 0) {
      lines.push(`  ${totals.clamped} text block(s) trimmed to ${opts.maxTextChars} chars (marker left in place)`)
    }
    if (overBudget.length > 0) {
      lines.push(
        `  WARNING: ${overBudget.length} conversation(s) still exceed ~${formatTokens(CONTEXT_ADVISORY_TOKENS)} tokens of context.`,
        '  Compaction can only summarise a history it can replay, so a session this large may refuse',
        '  new turns. Re-import those ids with a tighter budget, for example:',
        `    /import-codex --session <id> --max-text-chars 65536 --max-tool-output 4000`,
      )
    }
