...
const USAGE = [
  ...
  '  --list               show what is available in range, then stop',
  '  --since-hours N      select conversations started within N hours (default 24)',
  '  --session ID         import one Codex session id (repeatable)',
  '  --max-tool-output N  truncate each tool output to N chars (default 4000, 0 = keep all)',
  '  --dry-run            convert and verify only; write nothing',
  '',
  'Run --list first to pick a scope: it prints each conversation with its full',
  'session id, time span, working directory, and opening prompt.',
  '',
...
function parseInput(rawInput) {
  const opts = { sinceHours: 24, sessionIds: [], maxToolOutput: 4000, dryRun: false, list: false }
