...
/**
 * Select the conversation prefix Codex itself still had in context.
 *
 * A rollout is an append-only log, but Codex does not replay it: each
 * `compacted` record replaces everything before it with the summary it carries
 * in `replacement_history`. Importing the whole log therefore imports history
 * the model can no longer see — and a long conversation can be hundreds of
 * times larger than its own current window.
 *
 * The window keeps the last `compacted` record and everything after it: that
 * record *is* the current context, and the tail is what happened since. Earlier
 * snapshots stay dropped, because their content is summarised by the last one.
 *
 * @param all - chronologically sorted, deduplicated records.
 * @param enabled - false keeps the full replay (`--full-history`).
 * @returns the records to convert plus what the window dropped.
 */
export function selectHistoryWindow(all, enabled) {
  const full = () => ({
    records: all,
    window: { applied: false, droppedRecords: 0, droppedChars: 0, boundary: undefined, boundaryIndex: -1 },
  })
  if (enabled !== true) return full()
  const lastCompaction = all.findLast((record) => record?.type === 'compacted')
  if (lastCompaction === undefined) return full()
  const boundaryIndex = all.indexOf(lastCompaction)
  // A compaction on the first record has nothing in front of it to drop, so the
  // window is the whole log and reporting one would be misleading.
  if (boundaryIndex <= 0) {
    return {
      records: all,
      window: { applied: false, droppedRecords: 0, droppedChars: 0, boundary: undefined, boundaryIndex },
    }
  }
  let droppedChars = 0
  for (const record of all.slice(0, boundaryIndex)) {
    try {
      droppedChars += JSON.stringify(record).length
    } catch {
      // A record that cannot be serialised cannot be sized; the count is a
      // diagnostic, never a correctness input.
    }
  }
  return {
    records: all.slice(boundaryIndex),
    window: {
      applied: true,
      droppedRecords: boundaryIndex,
      droppedChars,
      boundary: typeof lastCompaction.timestamp === 'string' ? lastCompaction.timestamp : undefined,
      boundaryIndex,
    },
  }
}
 ...
 *
 * @param options.historyWindow - restrict the conversation to Codex's own
 *   current compaction window (see {@link selectHistoryWindow}).
 * @returns `records` for conversion, `allRecords` — every prepared record before
 *   any window, for callers that need conversation-level metadata the window may
 *   have dropped — plus the window outcome in `window`.
 */
export function prepareRecords(segments, options = {}) {
  const all = []
  ...
  // Conversation-level metadata — cwd, model, title, createdAt — lives in the
  // records a compaction window drops, so the full list is kept beside the
  // windowed one and offered to callers through `allRecords`.
  const everyRecord = [...all]
  const windowed = selectHistoryWindow(all, options.historyWindow)
