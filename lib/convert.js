...
export function buildRecords(segments, sessionId, opts = {}) {
  ...
  const prepared = prepareRecords(segments)
  const all = prepared.records
  const { duplicateRecords, generatedImageKeys } = prepared
  const { outcomes: toolOutcomes, queues: toolOutcomeQueues } = collectToolOutcomeData(all)
  ...
  const rawCallCounts = collectRawCallCounts(all)
