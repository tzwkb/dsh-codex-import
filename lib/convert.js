  ...
  for (const convo of conversations) {
    for (const segment of convo.segments) {
      for (const record of segment.records) {
        ...
        for (const image of imagesOf(record.payload.content)) {
...
export function buildRecords(segments, sessionId, opts = {}) {
  const maxToolOutput = opts.maxToolOutput ?? 4000
  ...
  const stats = { truncated: 0, reasoning: 0, injected: 0, toolCalls: 0, synthesized: 0, imagesImported: 0, imagesSkipped: 0 }
  ...
  for (const r of all) {
    ...
    // `compacted`, `world_state`, `turn_context`, `token_usage_record` and
    ...
    switch (payload.type) {
      case 'message': {
        ...
        if (role === 'user') {
          ...
          if ((human === undefined || human.length === 0) && images.length === 0) {
            stats.injected += 1
            break
          }
          // Preserve the message's own block order, and keep an image-only
          // message: dropping it would silently delete the whole turn.
          const blocks = []
          if (human !== undefined && human.length > 0) blocks.push({ type: 'text', text: human })
...
export async function runImport({
  root, sinceHours = 24, sessionIds = [], maxToolOutput = 4000,
