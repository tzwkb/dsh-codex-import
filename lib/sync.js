...
import { join, dirname, relative, resolve, sep, parse, isAbsolute } from 'node:path'
import { readSessionLog } from './verify.js'

const STATE_VERSION = 1
const MANIFEST_VERSION = 2
// macOS presents the system temporary tree as `/var/folders/...` while `/var`
// itself is a protected alias to `/private/var`.  The default `os.tmpdir()`
// therefore necessarily passes through `/var`; rejecting it would make the
// plugin fail before its first import on a stock Mac.  These are OS-owned
// aliases, rather than user-configurable links, and are the only exceptions
// to the ancestor check below.
const PLATFORM_SYMLINK_ALIASES = new Set(['/tmp', '/var', '/var/tmp'])
...
function writeAtomic(path, contents, mode = 0o600) {
  ...
  assertSafeRoot(parent, 'metadata path')
  // Metadata is part of the ownership boundary.  Replacing a symlink here
  // would not follow its target, but it would silently destroy an operator's
  // link and could make a later run trust a file outside the sessions tree.
  // Refuse the target explicitly so every metadata write has the same
  // no-follow semantics as session publication.
  assertRegularFileTarget(path, 'metadata target')
  mkdirSync(parent, { recursive: true, mode: 0o700 })
  const temp = `${path}.tmp-${process.pid}-${Date.now()}-${randomUUID()}`
  try {
    writeFileSync(temp, contents, { mode, flag: 'wx' })
    chmodSync(temp, mode)
    // Re-check immediately before the rename.  A concurrently-created
    // symlink is still replaced atomically rather than followed, but refusing
    // it gives the caller a safe, observable outcome instead of erasing the
    // link unexpectedly.
    assertRegularFileTarget(path, 'metadata target')
...
export function readState(liveRoot) {
  try {
    const target = statePath(liveRoot)
    const stat = readLinkSafe(target)
    if (stat?.isSymbolicLink() || stat !== undefined && !stat.isFile()) return { version: STATE_VERSION, sessions: {} }
...
/** Refuse metadata directories/files that would make a sync non-transactional. */
function assertMetadataTargetsSafe(liveRoot) {
  const metadata = metadataRoot(liveRoot)
  assertSafeRoot(metadata, 'metadata path')
  const files = [statePath(liveRoot), manifestPath(liveRoot), legacyManifestPath(liveRoot)]
  for (const path of files) assertRegularFileTarget(path, 'metadata target')
  const directories = [
    join(metadata, 'codex-import-manifests'),
    join(metadata, 'codex-import-backups'),
    join(metadata, 'codex-import-rollback-results'),
  ]
  for (const path of directories) assertDirectoryTarget(path, 'metadata directory')
}
...
/** Check a file destination without following or replacing a symlink. */
function assertRegularFileTarget(path, label) {
  const stat = readLinkSafe(path)
  if (stat?.isSymbolicLink()) throw new Error(`the ${label} is a symbolic link: ${path}`)
  if (stat !== undefined && !stat.isFile()) throw new Error(`the ${label} is not a regular file: ${path}`)
  return stat
}

/** Check a directory destination without following or replacing a symlink. */
function assertDirectoryTarget(path, label) {
  const stat = readLinkSafe(path)
  if (stat?.isSymbolicLink()) throw new Error(`the ${label} is a symbolic link: ${path}`)
  if (stat !== undefined && !stat.isDirectory()) throw new Error(`the ${label} is not a directory: ${path}`)
  return stat
}

function isSymlink(path) {
  return readLinkSafe(path)?.isSymbolicLink() === true
    && !PLATFORM_SYMLINK_ALIASES.has(resolve(path))
}

/**
 * Inspect the path as the kernel will traverse it, before `resolve()` folds
 * away `..` components. Without this lexical walk, a path such as
 * `safe-link/../target` could hide a user symlink from the ancestor check.
 */
function assertNoSymlinkComponents(path, label) {
  const text = String(path)
  const absolute = isAbsolute(text) ? text : join(process.cwd(), text)
  ...
  for (const part of absolute.slice(parsed.root.length).split(/[\\/]+/).filter(Boolean)) {
    ...
    const stat = readLinkSafe(current)
    if (stat?.isSymbolicLink() && !PLATFORM_SYMLINK_ALIASES.has(resolve(current))) {
      throw new Error(`the ${label} contains a symbolic-link component: ${path}`)
    }
  }
}
...
function pathForKey(root, key) {
  ...
  const base = basePath === sep ? sep : `${basePath}${sep}`
...
function replaceLog(from, to) {
  ...
  assertRegularFileTarget(target, 'destination session log')
  assertSafeRoot(to, 'destination session directory')
...
function installSession(from, to) {
  ...
  assertSafeRoot(parent, 'destination parent')
  mkdirSync(parent, { recursive: true, mode: 0o700 })
...
export function assertSafeRoot(root, label = 'sessions root') {
  if (typeof root !== 'string' || root.trim().length === 0) {
    throw new Error(`the ${label} must be a non-empty path`)
  }
  assertNoSymlinkComponents(root, label)
  const base = resolve(root)
  const stat = readLinkSafe(base)
  if (stat !== undefined && (stat.isSymbolicLink() || !stat.isDirectory())) {
    if (!(stat.isSymbolicLink() && PLATFORM_SYMLINK_ALIASES.has(base))) {
      throw new Error(`the ${label} is not a regular directory: ${root}`)
    }
  }
  // Walk through existing ancestors and newly-created components. This closes
  // the less obvious case where `root` itself does not exist yet but its parent
  // is a user symlink. Stable OS aliases are the only intentional exception.
  let current = base
  while (true) {
    const currentStat = readLinkSafe(current)
    if (currentStat !== undefined) {
      if (currentStat.isSymbolicLink() && !PLATFORM_SYMLINK_ALIASES.has(current)) {
        throw new Error(`the ${label} contains a symbolic-link ancestor: ${root}`)
      }
    }
...
export function syncSessions(scratchRoot, liveRoot, results, force = false) {
  assertSafeRoot(scratchRoot, 'scratch root')
  ...
  // Fail before touching any session when ownership metadata is redirected or
  // malformed.  Otherwise a batch could install several logs and only then
  // discover that its state/manifest destination was unsafe, leaving a
  // publish with no trustworthy rollback record.
  try {
    assertMetadataTargetsSafe(liveRoot)
  } catch (error) {
    // Keep the historical reconcile API (a bucketed refusal rather than a
    // per-session throw) while still guaranteeing that no session was
    // published before the global metadata check completed.
    buckets.refused.push(refusal('<metadata>', String(error?.message ?? error)))
    return buckets
  }
  const hashFor = new Map()
  for (const result of Array.isArray(results) ? results : []) {
    if (result === null || typeof result !== 'object' || typeof result.dir !== 'string') continue
  ...
  let projectEntries
  try {
    projectEntries = readdirSync(scratchRoot, { withFileTypes: true })
  } catch (error) {
    buckets.refused.push(refusal('<scratch>', `scratch root is unavailable (${String(error?.message ?? error)})`))
  ...
  for (const projectDir of projectEntries) {
    ...
    let sessionEntries
    try {
      sessionEntries = readdirSync(join(scratchRoot, projectDir.name), { withFileTypes: true })
    } catch (error) {
      buckets.refused.push(refusal(projectDir.name, `scratch project is unavailable (${String(error?.message ?? error)})`))
      continue
    }
...
function readManifest(path) {
  assertRegularFileTarget(path, 'rollback manifest')
