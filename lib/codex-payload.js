...
const FAILED_STATUSES = new Set(['failed', 'error', 'errored', 'cancelled', 'canceled', 'incomplete', 'timeout', 'aborted', 'rejected', 'declined'])
...
export function collectToolOutcomes(records) {
  ...
  for (const record of records) {
    ...
    const callId = typeof rawCallId === 'string' && rawCallId.length > 0
      ? rawCallId
    ...
    const previous = outcomes.get(callId)
