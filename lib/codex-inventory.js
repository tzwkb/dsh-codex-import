 ...
 */
import {
  findRollouts, readRecordStream, readRecords, stringId, filenameIds,
...
/**
 * Render a record timestamp for a listing.
 *
 * Rollout records are UTC while Codex names files in local time; printing the
 * raw ISO string makes a listing disagree with the clock the user just looked
 * at, so both surfaces format local wall-clock minutes instead.
 */
...
export function collectConversationRefs(rollouts, sessionIds = []) {
  ...
  for (const file of rollouts) {
    ...
    // A paginated rollout names its page only in the filename (`<root>_<page>`),
    // so Codex's own stores can hand a caller an id the metadata never mentions.
    for (const id of filenameIds(file.path)) conversation.sourceIds.add(id)
